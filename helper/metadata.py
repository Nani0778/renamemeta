...
async def change_metadata(file, dirpath, key):
    LOGGER.info(f"Starting metadata modification for file: {new_file_name}")
    temp_file = f"{new_file_name}.temp.mkv"
    ...
    if process.returncode != 0:
        ...
        LOGGER.error(err)
        LOGGER.error(f"Error modifying metadata for file: {new_file_name}")
        return file

    os.replace(temp_file_path, full_file_path)
    LOGGER.info(f"Metadata modified successfully for file: {new_file_name}")
    return file
